...
import { resetRoot, navigate, goBack } from '~/navigation';
...
const reactotron = Reactotron.configure({
  ...
  host: '192.168.1.75', // Replace this ID with your local IP address
  onConnect: () => {
...
reactotron.onCustomCommand<[{ name: 'route'; type: ArgType.String }]>({
  ...
  handler: (args) => {
    ...
    if (route) {
      ...
      // this should be tied to the navigator, but since this is for debugging,
      // we can navigate to illegal routes
      navigate(route as Href<string | object>);
    } else {
...
try {
  ...
  console.tron.error('Error connecting to Reactotron', error);
}
