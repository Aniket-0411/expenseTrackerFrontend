 ...
 */
const RootStoreContext = createContext<RootStore>(rootStoreInit);
 ...
 * @param {() => void | Promise<void>} callback - an optional callback that's
 *  invoked once the store is ready
...
export const useInitialRootStore = (callback?: () => void | Promise<void>) => {
  ...
  useEffect(() => {
    ...
    return () => {
      // cleanup
      if (unsubscribeFn !== undefined) unsubscribeFn();
    };
