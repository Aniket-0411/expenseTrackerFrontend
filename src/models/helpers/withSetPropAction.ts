 ...
 *
 * @deprecated - This will be removed since wer are going to remove mobx.
 */
export const withSetPropAction = <T extends IStateTreeNode>(mobXInstance: T) => ({
  // generic setter for all properties
  setProp<K extends keyof SnapshotIn<T>, V extends SnapshotIn<T>[K]>(field: K, newValue: V) {
    const mstInstance = mobXInstance;
